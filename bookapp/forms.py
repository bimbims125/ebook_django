...
from .models import BookSearch, AuthorSearch
from django.contrib.auth.forms import UserCreationForm
from django.contrib.auth.models import User

class BookSearchForm(forms.ModelForm):
    name_of_book = forms.CharField(max_length=100, widget=forms.TextInput(attrs={
        'class':'form-control', 'type':'search', 'placeholder':'Masukan Nama Buku', 'aria-label':'Search', 'data-width':'250'
    }))
    class Meta:
        model = BookSearch
        fields = ['name_of_book',]
...
class AuthorSearchForm(forms.ModelForm):
    name_of_author = forms.CharField(max_length=100, widget=forms.TextInput(attrs={
        'class':'form-control', 'type':'search', 'placeholder':'Masukan Pengarang', 'aria-label':'Search', 'data-width':'250'
    }))
    class Meta:
        model = AuthorSearch
        fields = ['name_of_author',]
